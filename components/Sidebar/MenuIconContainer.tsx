...
const MenuIconContainer = ({children}: MenuItemProps) => {
    ...
    return (
        ...
        <div onClick={() => setActive(!isActive)} className={`p-[10px] my-[30px] cursor-pointer rounded-lg ${isActive ? 'hover:bg-slate-300 bg-gradient-to-t from-[#EA9769] to-[#EA6969]' : 'hover:bg-slate-700'}`}>
            {children}
        </div>
    ) 
}
